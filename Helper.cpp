...
void JQ()
{
	...
	printf("请输入年份以及要查询的节气编号：Year Index>");
	scanf("%d %d",&iYear,&iIndex);
	wMonth=((iIndex+1)/2)%12+1;//转换到月份
	sYear=(short)(iYear);
	do 
	{
		...
		if (ret==0)
		{
			printf("输入错误，请重新输入年份以及要查询的节气编号：Year Index>");
			scanf("%d %d",&iYear,&iIndex);
	...
	{
		printf("%d年 %s 时间为 %d月%d日，农历 ",sYear,Jieqi[iIndex-1],wMonth,wDay);
